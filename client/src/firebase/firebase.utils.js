...
export const convertCollectionsSnapshotToMap= (collections) => {
  const transformtedCollection = collections.docs.map(doc => {
  ...
  return transformtedCollection.reduce((accumulator, collection) => {
...
export const getCurrentUser = () => {
  return new Promise((resolve, reject) => {
    const unsubscribe = auth.onAuthStateChanged(userAuth => { //api listenere
      unsubscribe();
      resolve(userAuth);
    }, reject)
  })
